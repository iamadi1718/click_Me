...
class SocketManager {
  ...
  final _viewerJoinController = StreamController<Map<String, dynamic>>.broadcast();
  Stream<Map<String, dynamic>> get onViewerJoin => _viewerJoinController.stream;

  final _viewerLeaveController = StreamController<Map<String, dynamic>>.broadcast();
  Stream<Map<String, dynamic>> get onViewerLeave => _viewerLeaveController.stream;

  final _viewerCountController = StreamController<int>.broadcast();
  Stream<int> get onViewerCount => _viewerCountController.stream;

  final _receiveMessageController = StreamController<Map<String, dynamic>>.broadcast();
  Stream<Map<String, dynamic>> get onReceiveMessage => _receiveMessageController.stream;
  ...
  final _liveHeartController = StreamController<Map<String, dynamic>>.broadcast();
  ...
  Stream<Map<String, dynamic>> get onLiveReaction => _liveReactionController.stream;
...
Stream<Map<String, dynamic>> get onIncomingCall =>
  ...
  void init() {
    print("[DEBUG SocketManager] init() called.");
    ...
    print("SocketManager: Initializing connection to ${Api.baseUrl} with token: $token");
    ...
    socket!.onAny((event, data) {
      if (event != 'ping' && event != 'pong') { // Avoid heartbeat spam
    ...
    socket!.onConnect((_) {
      ...
      if (_activeStreamId != null && _activeUserId != null) {
        print("[DEBUG SocketManager] Auto-rejoining active room $_activeStreamId");
        if (_isActiveHost) {
          socket?.emit('live_started', {'streamId': _activeStreamId});
        }
        socket?.emit('join_live', {'streamId': _activeStreamId, 'userId': _activeUserId});
      }
  ...
  _callAcceptedController.add(parseData(data));
    ...
    // Bind Listeners
    socket!.on('live_started', (data) => _liveStartedController.add(parseData(data)));
    socket!.on('live_ended', (data) => _liveEndedController.add(parseData(data)));
    socket!.on('viewer_join', (data) => _viewerJoinController.add(parseData(data)));
    socket!.on('viewer_leave', (data) => _viewerLeaveController.add(parseData(data)));
    ...
    socket!.on('receive_message', (data) => _receiveMessageController.add(parseData(data)));
    socket!.on('new_message', (data) => _receiveMessageController.add(parseData(data)));
    socket!.on('new_comment', (data) => _receiveMessageController.add(parseData(data)));
    socket!.on('comment', (data) => _receiveMessageController.add(parseData(data)));
    ...
    socket!.on('live_heart', (data) => _liveHeartController.add(parseData(data)));
    socket!.on('live_reaction', (data) => _liveReactionController.add(parseData(data)));

    // WebRTC Signaling
    socket!.on('offer', (data) => _offerController.add(parseData(data)));
    socket!.on('answer', (data) => _answerController.add(parseData(data)));
    socket!.on('ice_candidate', (data) => _iceCandidateController.add(parseData(data)));
  ...
  void sendMessage(String streamId, String userId, String message) =>
      _safeEmit('send_message', {'streamId': streamId, 'userId': userId, 'message': message});
  ...
  void sendIceCandidate(Map<String, dynamic> data) => _safeEmit('ice_candidate', data);
